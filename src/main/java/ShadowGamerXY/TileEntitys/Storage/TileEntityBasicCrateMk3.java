...
public class TileEntityBasicCrateMk3 extends TileEntitySCE implements IInventory{

    private static final int SLOTS_PER_BLOCK = 32;
    private ItemStack[] inventory = new ItemStack[SLOTS_PER_BLOCK];

    private TileEntityBasicCrateMk3 master;
    ...
    public TileEntityBasicCrateMk3 getMaster(){
        initializeMultiblockIfNecessary();
        return master;
    }

    private void setMaster(TileEntityBasicCrateMk3 master, int storages){
        this.master = master;
    ...
    public void invalidate(){
        ...
        for(ForgeDirection d : ForgeDirection.VALID_DIRECTIONS) {
            ...
            if(te instanceof TileEntityBasicCrateMk3) {
                ((TileEntityBasicCrateMk3)te).master = null;
                ((TileEntityBasicCrateMk3)te).initializeMultiblockIfNecessary();
            }
        }
        for(ItemStack stack : inventory) {
            if(stack != null) worldObj.spawnEntityInWorld(new EntityItem(worldObj, xCoord, yCoord, zCoord, stack));
    ...
    private void initializeMultiblockIfNecessary(){
        if(master == null || master.isInvalid()) {
            List<TileEntityBasicCrateMk3> connectedStorages = new ArrayList<TileEntityBasicCrateMk3>();
            Stack<TileEntityBasicCrateMk3> traversingStorages = new Stack<TileEntityBasicCrateMk3>();
            TileEntityBasicCrateMk3 master = this;
            traversingStorages.add(this);
            while(!traversingStorages.isEmpty()) {
                TileEntityBasicCrateMk3 storage = traversingStorages.pop();
                if(storage.isMaster()) {
                    master = storage;
                }
                ...
                for(ForgeDirection d : ForgeDirection.VALID_DIRECTIONS) {
                    ...
                    if(te instanceof TileEntityBasicCrateMk3 && !connectedStorages.contains(te)) {
                        traversingStorages.add((TileEntityBasicCrateMk3)te);
                    }
                }
            }
            LogHelper.info("Setting master to " + master.xCoord + ", " + master.yCoord + ", " + master.zCoord + " for " + connectedStorages.size() + " blocks");
            for(TileEntityBasicCrateMk3 storage : connectedStorages) {
