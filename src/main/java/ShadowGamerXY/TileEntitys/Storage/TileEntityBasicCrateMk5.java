...
public class TileEntityBasicCrateMk5 extends TileEntitySCE implements IInventory{

    private static final int SLOTS_PER_BLOCK = 128;
    private ItemStack[] inventory = new ItemStack[SLOTS_PER_BLOCK];

    private TileEntityBasicCrateMk5 master;
    ...
    public TileEntityBasicCrateMk5 getMaster(){
        initializeMultiblockIfNecessary();
        return master;
    }

    private void setMaster(TileEntityBasicCrateMk5 master, int storages){
        this.master = master;
    ...
    public void invalidate(){
        ...
        for(ForgeDirection d : ForgeDirection.VALID_DIRECTIONS) {
            ...
            if(te instanceof TileEntityBasicCrateMk5) {
                ((TileEntityBasicCrateMk5)te).master = null;
                ((TileEntityBasicCrateMk5)te).initializeMultiblockIfNecessary();
            }
        }
        for(ItemStack stack : inventory) {
            if(stack != null) worldObj.spawnEntityInWorld(new EntityItem(worldObj, xCoord, yCoord, zCoord, stack));
    ...
    private void initializeMultiblockIfNecessary(){
        if(master == null || master.isInvalid()) {
            List<TileEntityBasicCrateMk5> connectedStorages = new ArrayList<TileEntityBasicCrateMk5>();
            Stack<TileEntityBasicCrateMk5> traversingStorages = new Stack<TileEntityBasicCrateMk5>();
            TileEntityBasicCrateMk5 master = this;
            traversingStorages.add(this);
            while(!traversingStorages.isEmpty()) {
                TileEntityBasicCrateMk5 storage = traversingStorages.pop();
                if(storage.isMaster()) {
                    master = storage;
                }
                ...
                for(ForgeDirection d : ForgeDirection.VALID_DIRECTIONS) {
                    ...
                    if(te instanceof TileEntityBasicCrateMk5 && !connectedStorages.contains(te)) {
                        traversingStorages.add((TileEntityBasicCrateMk5)te);
                    }
                }
            }
            LogHelper.info("Setting master to " + master.xCoord + ", " + master.yCoord + ", " + master.zCoord + " for " + connectedStorages.size() + " blocks");
            for(TileEntityBasicCrateMk5 storage : connectedStorages) {
