...
public class TileEntityBasicCrateMk5Deep extends TileEntitySCE implements IInventory{

    private static final int SLOTS_PER_BLOCK = 600;
    private ItemStack[] inventory = new ItemStack[SLOTS_PER_BLOCK];

    private TileEntityBasicCrateMk5Deep master;
    ...
    public TileEntityBasicCrateMk5Deep getMaster(){
        initializeMultiblockIfNecessary();
        return master;
    }

    private void setMaster(TileEntityBasicCrateMk5Deep master, int storages){
        this.master = master;
    ...
    public void invalidate(){
        ...
        for(ForgeDirection d : ForgeDirection.VALID_DIRECTIONS) {
            ...
            if(te instanceof TileEntityBasicCrateMk5Deep) {
                ((TileEntityBasicCrateMk5Deep)te).master = null;
                ((TileEntityBasicCrateMk5Deep)te).initializeMultiblockIfNecessary();
            }
        }
        for(ItemStack stack : inventory) {
            if(stack != null) worldObj.spawnEntityInWorld(new EntityItem(worldObj, xCoord, yCoord, zCoord, stack));
    ...
    private void initializeMultiblockIfNecessary(){
        if(master == null || master.isInvalid()) {
            List<TileEntityBasicCrateMk5Deep> connectedStorages = new ArrayList<TileEntityBasicCrateMk5Deep>();
            Stack<TileEntityBasicCrateMk5Deep> traversingStorages = new Stack<TileEntityBasicCrateMk5Deep>();
            TileEntityBasicCrateMk5Deep master = this;
            traversingStorages.add(this);
            while(!traversingStorages.isEmpty()) {
                TileEntityBasicCrateMk5Deep storage = traversingStorages.pop();
                if(storage.isMaster()) {
                    master = storage;
                }
                ...
                for(ForgeDirection d : ForgeDirection.VALID_DIRECTIONS) {
                    ...
                    if(te instanceof TileEntityBasicCrateMk5Deep && !connectedStorages.contains(te)) {
                        traversingStorages.add((TileEntityBasicCrateMk5Deep)te);
                    }
                }
            }
            LogHelper.info("Setting master to " + master.xCoord + ", " + master.yCoord + ", " + master.zCoord + " for " + connectedStorages.size() + " blocks");
            for(TileEntityBasicCrateMk5Deep storage : connectedStorages) {
