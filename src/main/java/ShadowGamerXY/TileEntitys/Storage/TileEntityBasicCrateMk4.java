...
public class TileEntityBasicCrateMk4 extends TileEntitySCE implements IInventory{

    private static final int SLOTS_PER_BLOCK = 64;
    private ItemStack[] inventory = new ItemStack[SLOTS_PER_BLOCK];

    private TileEntityBasicCrateMk4 master;
    ...
    public TileEntityBasicCrateMk4 getMaster(){
        initializeMultiblockIfNecessary();
        return master;
    }

    private void setMaster(TileEntityBasicCrateMk4 master, int storages){
        this.master = master;
    ...
    public void invalidate(){
        ...
        for(ForgeDirection d : ForgeDirection.VALID_DIRECTIONS) {
            ...
            if(te instanceof TileEntityBasicCrateMk4) {
                ((TileEntityBasicCrateMk4)te).master = null;
                ((TileEntityBasicCrateMk4)te).initializeMultiblockIfNecessary();
            }
        }
        for(ItemStack stack : inventory) {
            if(stack != null) worldObj.spawnEntityInWorld(new EntityItem(worldObj, xCoord, yCoord, zCoord, stack));
    ...
    private void initializeMultiblockIfNecessary(){
        if(master == null || master.isInvalid()) {
            List<TileEntityBasicCrateMk4> connectedStorages = new ArrayList<TileEntityBasicCrateMk4>();
            Stack<TileEntityBasicCrateMk4> traversingStorages = new Stack<TileEntityBasicCrateMk4>();
            TileEntityBasicCrateMk4 master = this;
            traversingStorages.add(this);
            while(!traversingStorages.isEmpty()) {
                TileEntityBasicCrateMk4 storage = traversingStorages.pop();
                if(storage.isMaster()) {
                    master = storage;
                }
                ...
                for(ForgeDirection d : ForgeDirection.VALID_DIRECTIONS) {
                    ...
                    if(te instanceof TileEntityBasicCrateMk4 && !connectedStorages.contains(te)) {
                        traversingStorages.add((TileEntityBasicCrateMk4)te);
                    }
                }
            }
            LogHelper.info("Setting master to " + master.xCoord + ", " + master.yCoord + ", " + master.zCoord + " for " + connectedStorages.size() + " blocks");
            for(TileEntityBasicCrateMk4 storage : connectedStorages) {
