...
public class TileEntityBasicCrate extends TileEntitySCE implements IInventory{

    private static final int SLOTS_PER_BLOCK = 1;
    private ItemStack[] inventory = new ItemStack[SLOTS_PER_BLOCK];

    private TileEntityBasicCrate master;
    ...
    public TileEntityBasicCrate getMaster(){
        initializeMultiblockIfNecessary();
        return master;
    }

    private void setMaster(TileEntityBasicCrate master, int storages){
        this.master = master;
    ...
    public void invalidate(){
        ...
        for(ForgeDirection d : ForgeDirection.VALID_DIRECTIONS) {
            ...
            if(te instanceof TileEntityBasicCrate) {
                ((TileEntityBasicCrate)te).master = null;
                ((TileEntityBasicCrate)te).initializeMultiblockIfNecessary();
            }
        }
        for(ItemStack stack : inventory) {
            if(stack != null) worldObj.spawnEntityInWorld(new EntityItem(worldObj, xCoord, yCoord, zCoord, stack));
    ...
    private void initializeMultiblockIfNecessary(){
        if(master == null || master.isInvalid()) {
            List<TileEntityBasicCrate> connectedStorages = new ArrayList<TileEntityBasicCrate>();
            Stack<TileEntityBasicCrate> traversingStorages = new Stack<TileEntityBasicCrate>();
            TileEntityBasicCrate master = this;
            traversingStorages.add(this);
            while(!traversingStorages.isEmpty()) {
                TileEntityBasicCrate storage = traversingStorages.pop();
                if(storage.isMaster()) {
                    master = storage;
                }
                ...
                for(ForgeDirection d : ForgeDirection.VALID_DIRECTIONS) {
                    ...
                    if(te instanceof TileEntityBasicCrate && !connectedStorages.contains(te)) {
                        traversingStorages.add((TileEntityBasicCrate)te);
                    }
                }
            }
            //LogHelper.info("Setting master to " + master.xCoord + ", " + master.yCoord + ", " + master.zCoord + " for " + connectedStorages.size() + " blocks");
            for(TileEntityBasicCrate storage : connectedStorages) {
